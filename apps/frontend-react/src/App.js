...
{activeTab === 'activities' && (
  ...
  <div className="max-w-[1600px] mx-auto grid grid-cols-1 lg:grid-cols-12 gap-8 h-[850px] animate-in fade-in duration-500">
    ...
    {/* LINKE SPALTE: XProzess Konfiguration (4 von 12 Spalten) */}
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 shadow-xl">
        ...
        <div className="space-y-4">
              ...
              {genFiles.map((file, index) => (
                <div key={index} className="flex items-center justify-between p-2 border-b border-slate-800 bg-slate-900/50 text-[10px]">
                  <div className="flex items-center gap-2 truncate text-slate-300">
                    <FileIcon fileName={file.name} size={12} className="text-blue-400" />
          ...
          <button 
            ...
            disabled={isGenerating || genFiles.length === 0} 
          ...
          >
            ...
            {isGenerating ? 'KI analysiert...' : 'XProzess generieren'}
          </button>
        </div>
      </div>
      
      {generationError && (
        <div className="bg-red-500/10 border border-red-500/20 text-red-400 p-4 rounded-lg text-xs flex items-start gap-3">
    ...
    {/* RECHTE SPALTE: BPMN-Viewer ODER Review-Maske (8 von 12 Spalten) */}
          ...
          <div className="flex gap-6">
            ...
            {steckbriefDraft && (
              ...
              >
                ...
                <span className="bg-blue-500/20 text-blue-400 text-[10px] px-1.5 py-0.5 rounded-full border border-blue-500/30 ml-1">KI ENTWURF</span>
              </button>
            )}
          </div>
          ...
          <div className="flex gap-2">
            {generatedXml && (
              <a 
                ...
                className="text-xs flex items-center gap-1.5 bg-blue-600 hover:bg-blue-500 px-3 py-1.5 rounded text-white transition shadow-md"
              >
            ...
            <div className="flex-1 flex flex-col overflow-hidden animate-in fade-in slide-in-from-right-4 duration-300">
                    ...
                    {steckbriefDraft.prozessteilnehmer?.map((tp, i) => (
                      <div key={i} className="flex items-center gap-3 p-3 bg-slate-900/50 border border-slate-800 rounded-lg">
                        ...
                        <div>
                          ...
                          <div className="text-[10px] text-slate-500">Code: {tp.rolle?.code || 'n/a'}</div>
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              <div className="p-6 bg-slate-900 border-t border-slate-800 flex gap-4 shrink-0">
                <button 
                  ...
                  onClick={() => { alert("Daten übernommen!"); }}
                >
                  <Save size={18} /> Daten übernehmen & Finalisieren
                </button>
                <button onClick={() => setIsReviewing(false)} className="px-8 py-3.5 border border-slate-700 text-slate-400 rounded-xl font-medium transition-all">Abbrechen</button>
              </div>
            </div>
          ) : (
            /* BPMN VIEWER */
            <div className="w-full h-full flex flex-col bg-white">
              {generatedXml ? (
                ...
                <div className="flex-1 flex flex-col items-center justify-center text-slate-600 p-20 text-center">
                  <FileJson size={64} className="mx-auto mb-4 text-slate-700 opacity-20" />
                  <p className="text-sm">Hier erscheint nach der Analyse das BPMN-Modell.</p>
                </div>
              )}
            </div>
          )}
